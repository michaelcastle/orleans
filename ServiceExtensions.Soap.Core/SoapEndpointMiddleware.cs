...
namespace ServiceExtensions.Soap.Core
{
    public class SoapEndpointMiddleware
    {
        ...
        private static string GetSoapAction(HttpContext httpContext, Message requestMessage, System.Xml.XmlDictionaryReader reader)
        {
        ...
        private async Task<Message> ProcessOperation(HttpContext httpContext, IServiceProvider serviceProvider)
        {
            ...
            {
                responseMessage = WriteErrorResponseMessage(ex, StatusCodes.Status500InternalServerError, serviceProvider, httpContext, requestMessage);
                return responseMessage;
            }
            ...
            using (var reader = requestMessage.GetReaderAtBodyContents())
            {
                ...
                _logger.LogDebug("{method}: {soapAction}", "SoapEndpointMiddleware.ProcessOperation", soapAction);
                ...
                try
                {
                    ...
                    var responseProvider = serviceProvider.GetService<IResponseMessageService>();
                    responseMessage = responseProvider == null ? new CustomMessage(responseMessage) : responseProvider.GetResponse(requestMessage.Headers, responseMessage);
                ...
                catch (Exception exception)
                {
                    ...
                    responseMessage = WriteErrorResponseMessage(exception, StatusCodes.Status500InternalServerError, serviceProvider, httpContext, requestMessage);
                }
            }
        ...
        private object[] GetRequestArguments(Message requestMessage, System.Xml.XmlDictionaryReader xmlReader, OperationDescription operation)
        {
            ...
            if (_serializer == SoapSerializer.StringBodyDataContractSerializer)
            {
                var body = xmlReader.ReadOuterXml();
                arguments[0] = body;
                return arguments;
            }
        ...
        {
            _logger.LogError(exception, "Error: WriteErrorResponseMessage");

            var bodyWriter = GetOperationErrorBodyWriter(exception, serviceProvider, httpContext, requestMessage);
            if (bodyWriter == null)
            {
                bodyWriter = GetDefaultErrorBodyWriter(exception, serviceProvider);
            }
        ...
        private BodyWriter GetOperationErrorBodyWriter(Exception exception, IServiceProvider serviceProvider, HttpContext httpContext, Message requestMessage)
        {
            var bodyWriterService = serviceProvider.GetService<IOperationErrorBodyWriterService>();
            if (bodyWriterService == null)
            {
                return null;
            }
            ...
            using (var reader = requestMessage.GetReaderAtBodyContents())
            {
                ...
                requestMessage.Headers.Action = operation.SoapAction;

                return new ServiceBodyWriter(_serializer, operation, operation.ReturnName, bodyWriterService.GetOperationBodyWriter(exception), null);
            }
        }

        private static BodyWriter GetDefaultErrorBodyWriter(Exception exception, IServiceProvider serviceProvider)
        {
            ...
            return new FaultBodyWriter(new Fault(faultDetail) { FaultString = errorText });
        }
        ...
        /// </returns>
        private static object ExtractFaultDetail(
                    Exception exception)
        {
            try
            {
